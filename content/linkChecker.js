    ...
    let links = document.getElementsByTagName('a');
        ...
        for (let i = 0; i < links.length; i++) {
            ...
            if (links[i].host === window.location.host) {
                internalLinks.push(links[i].href)
            } else {
                externalLinks.push(links[i].href)
            }
            if (links[i].rel === 'nofollow') {
                noFollowLinks.push(links[i].href)
            } else {
                doFollowLinks.push(links[i].href)
            }

        }
        chrome.runtime.sendMessage({
            message: 'linkCheckerData',
            data: {
                internalLinks,
                externalLinks,
                doFollowLinks,
...
const linkCheckerData_bg = async (response) => {
